...
func (c *sclient) logConnectivityf(format string, v ...any) {
	// Include a prefix for ease of grepping the logs.
	const prefix = "derper connectivity: "
	if c.canMesh {
		// Mesh peers have their connectivity logs logged at info level since
		// failures are much more severe.
...
func (s *Server) ExpVar() expvar.Var {
	...
	m.Set("gauge_watchers", s.expVarFunc(func() any { return s.watchers.Len() }))
